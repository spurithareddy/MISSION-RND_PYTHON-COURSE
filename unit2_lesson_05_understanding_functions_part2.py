...
def test_function_call_with_keyword_arguments():
    assert [10,0,3,4] == demo(10,0)  # missing one positional argument(second)
    assert [10,20,3,4] == demo(10, 20)
    assert [10,20,30,4] == demo(10, 20, 30)
    assert [10,20,3,4] == demo(first=10,second=20)   # missing one positional argument(first)
    assert [10,20,30,4] == demo(first=10,second=20, third=30)  # missing one positional argument(first)
    assert [10,20,30,4] == demo(first=10,second=20,third=30)  # missing one positional argument(second)
    assert [10,20,30,4] == demo(10,20,third=30)
    assert [10,20,3,4] == demo(10,second=20)  # is this allowed? correct and uncomment (positional argument follows keyword argument)
...
def test_function_with_keyword_args():
    ...
    assert {} == demo_with_keyword_args(name="jack")  # what do you observe here?(named argument already exists as a parameter)
...
def simple_format(format, *args):
    ...
    for i,name in enumerate(args):
        format = format.replace("%" + str(i), name)
    return format
    pass
...
- positional and keyword arguments and named arguments 
- packing and unpacking arguments
- sting formatting and re module (regular expression)(methods like match,search,sub)
"""
