...
def new_list(input):
    ...
    for y in range(0, len(input)):
        li.append(len(set(input[y]).intersection('aeiou')))
    return li
# new functions end
...
def get_word_with_least_vowels(input):
    ...
    input = list(input)
    return input[(new_list(input).index(min(new_list(input))))]  # index returns the index of min(len)
...
def max_min_list(input):
    lix = []
    for c in range(0, len(input)):
        lix.append(len(input[c]))
    return lix
...
def get_min_max_words(input):
    ...
    """
    input = list(input)
    p = max_min_list(input)
    return input[p.index(min(p))], input[p.index(max(p))]
...
- python functions and function passing as an arguement to the other func
- default parameters
- about callable function (which returns boolean)
"""