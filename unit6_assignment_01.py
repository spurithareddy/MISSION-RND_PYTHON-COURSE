...
def generator_zip(seq1, seq2, *more_seqs):
    ...
    if not more_seqs:
        while True:
            res = tuple(map(next,(t, lt2)))
            if len(res) < 2:
                raise StopIteration
            yield res
    else:
        lt3 = tuple(map(iter,more_seqs))
        while True:
            res = tuple(map(next,(t,lt2,*lt3)))
            if len(res) < len(more_seqs)+2:
                raise StopIteration
            yield res
...
def test_generator_zip():
    ...
    gene = generator_zip(range(3), range(3), range(3))
    assert [(0, 0, 0), (1, 1, 1), (2, 2, 2)] == list(gene)
