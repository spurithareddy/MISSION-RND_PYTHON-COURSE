...
def get_reverse_mapping():
    ...
    """
    return {xp : chr(122-cp) for cp,xp in enumerate(string.ascii_lowercase)}
    pass
...
def get_lower_to_upper_dict():
    ...
    """
    return {xp : chr(65+cp) for cp,xp in enumerate(string.ascii_lowercase)}
...
def get_months():
    ...
    """
    xx = list(calendar.month_abbr)[1:]
    return {(count + 1): b for (count, b) in enumerate(xx)}
