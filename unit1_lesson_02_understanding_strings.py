...
def test_string_combine():
    ...
    hello = "Hello World"
    bye = "Goodbye moon"
    assert  bye[0:8] + hello[6:12]  == "Goodbye World"

def test_string_formatting():
    greeting = "Hello '{}'".format("learner")
    assert "Hello \'learner\'" == greeting

    truth = "{1} plus {1} makes {2}".format("one", "one", "two")
...
def test_string_membership():
    assert ('c' not in 'apple') == True  #is there a precedence issue here or something else?
    assert ('a' in 'apple') == True
...
def test_string_methods():
    ...
    assert 'zebra' == word.replace("hello", "zebra")
